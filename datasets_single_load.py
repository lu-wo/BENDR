...
class MultiTaskDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage: str=None):
        ...
        if stage == "fit" or stage is None:
            self.train = MultiTaskDataset(paths=self.train_paths, file_pattern=self.file_pattern, window_len=self.window_len)
            logging.info(f"Datamodule: Created training dataset with {len(self.train)} samples")
            self.val = MultiTaskDataset(paths=self.val_paths, file_pattern=self.file_pattern, window_len=self.window_len)
            logging.info(f"Datamodule: Created validation dataset with {len(self.val)} samples")

        if stage == "test" or stage is None:
            self.test = MultiTaskDataset(paths=self.test_paths, file_pattern=self.file_pattern, window_len=self.window_len)
